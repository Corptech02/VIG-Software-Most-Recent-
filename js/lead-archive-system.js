...
(function() {
    ...
    window.restoreArchivedLead = function(leadId) {
        ...
        // IMPORTANT: Save restored lead to server
        fetch('/api/leads', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
        ...
        .then(response => response.json())
        .then(data => {
            console.log('Lead restore status saved to server:', data);
        })
        .catch(error => {
            console.error('Failed to save restore status to server:', error);
            showNotification('Warning: Restore status may not persist after refresh', 'warning');
        });
